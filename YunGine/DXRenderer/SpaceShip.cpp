...
void SpaceShip::ObjectUpdate(const DirectX::XMMATRIX& world, const DirectX::XMMATRIX& view, const DirectX::XMMATRIX& projection)
{
	_world = world;
	_view = view;
	_proj = projection;
...
void SpaceShip::Render()
{
	...
	_textureSpaceShipMatrixVariable->SetMatrix(reinterpret_cast<float*>(&worldViewProj));
	...
	_textureSpaceShipTechnique->GetDesc(&techDesc);

	//랜더패스
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		_textureSpaceShipTechnique->GetPassByIndex(p)->Apply(0, _3DDeviceContext.Get());

		_3DDeviceContext->DrawIndexed(indexcount, 0, 0);
	}
}

void SpaceShip::ObjectSetting()
{
	HRESULT hr = S_OK;


	//Fbx파일을 읽어서 넘겨줘야함
	TexVertex SpaceShipVertex[] =
	{
		// 포지션, 텍스코드
		// 앞면
	...
	InitData.pSysMem = SpaceShipVertex;	// const 무효-> 초기화 데이터에 대한 포인터이다.
	...
	UINT SpaceShipindexcount = ARRAYSIZE(indices);
	indexcount = ARRAYSIZE(indices);

	D3D11_BUFFER_DESC indexBufferDesc;
	indexBufferDesc.Usage = D3D11_USAGE_IMMUTABLE;
	indexBufferDesc.ByteWidth = SpaceShipindexcount * sizeof(UINT);
...
void SpaceShip::BuildFX()
{
	...
	//D3DX11CreateEffectFromMemory(compiledShader->GetBufferPointer(), compiledShader->GetBufferSize(), 0, _3DDevice.Get(), _textureSpaceShipEffect.GetAddressOf());

	_Technique = _textureSpaceShipEffect->GetTechniqueByName("TextureTech");
	_textureSpaceShipMatrixVariable = _textureSpaceShipEffect->GetVariableByName("gWorldViewProj")->AsMatrix();
}

void SpaceShip::BuildVertexLayout()
{
	HRESULT hr = S_OK;
	...
	_textureSpaceShipTechnique->GetPassByIndex(0)->GetDesc(&passDesc);
...
void SpaceShip::BuildFxFromCso(ID3D11Device* device, const std::wstring& filename)
{
	...
	HRESULT hr = D3DX11CreateEffectFromMemory(&compiledShader[0], size,
		0, device, &_textureSpaceShipEffect);

	_textureSpaceShipTechnique = _textureSpaceShipEffect->GetTechniqueByName("TextureTech");
	_textureSpaceShipMatrixVariable = _textureSpaceShipEffect->GetVariableByName("gWorldViewProj")->AsMatrix();	// 행렬로 쓸꺼야
	_gworldMatrixVariable = _textureSpaceShipEffect->GetVariableByName("gWorld")->AsMatrix();	// 행렬로 쓸꺼야

	_diffuseMap = _textureSpaceShipEffect->GetVariableByName("gDiffuseMap")->AsShaderResource();

	BOOL res = _textureSpaceShipEffect->IsValid();	// 메모리가 쓰레기값이 아닌지를 확인하는 함수
	res = _textureSpaceShipTechnique->IsValid();
	res = _textureSpaceShipMatrixVariable->IsValid();
